...
//        count = 155;
//        ret = chars_len;
//        memcpy(buf, chars, (size_t) chars_len);
//    }
//    if(ret > 0) {
//        LOGE("ret %d", ret);
//    }
...
////    player->showWidth = avFmtCtx->streams[i]->codec->coded_width = 1280;
////    player->showHeight = avFmtCtx->streams[i]->codec->coded_height = 752;
//    player->showWidth = avFmtCtx->streams[i]->codec->width = 1280;
//    player->showHeight = avFmtCtx->streams[i]->codec->height = 752;
////    player->rate = avFmtCtx->streams[i]->r_frame_rate.num = 0;
////    for (; i < avFmtCtx->nb_streams; i++) {
//////    for (; i < 2; i++) {
////        LOGI("index: %d, codec_type:%d", i, avFmtCtx->streams[i]->codec->codec_type);//0 0
////        //流的类型
////        if (avFmtCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO &&
////            player->video_stream_idx == -1) {
////            player->video_stream_idx = i;//0
////            player->showWidth = avFmtCtx->streams[i]->codec->coded_height;//1280
////            player->showHeight = avFmtCtx->streams[i]->codec->coded_width;//752
////            player->rate = avFmtCtx->streams[i]->r_frame_rate.num;//0
////            LOGI("w:%d, h:%d index:%d, rate:%d", player->showWidth, player->showHeight, i,
////                 player->rate);
////        } else if (avFmtCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
////                   player->audio_stream_idx == -1) {
...
//
//    frame_delay = av_q2d(
//            player->input_format_ctx->streams[player->video_stream_idx]->codec->time_base);
...
////                                player->input_format_ctx->streams[player->video_stream_idx]->time_base,
////                                AV_TIME_BASE_Q);
////    LOGE("pts  %d", pts);
////    LOGE("time  %d", time);
...
////        pCodecCtx->me_method = 0;
////        pCodecCtx->ticks_per_frame = 60;
////        pCodecCtx->gop_size = 752;
////        pCodecCtx->sample_rate = 1;
////        pCodecCtx->idct_algo = 8;
